...
def s6_architecture(prs):
    ...
    boxes = [
        ...
        ("recalculate", "90 functions,\nExcel's coercion rules"),
        ("15 rules", "structural, correctness,\nhygiene — no model"),
...
def s8_proof(prs):
    s = slide(prs)
    eyebrow(s, "Proof")
    title(s, "Four numbers, all reproducible from the repository.", size=30)
    stats = [
        ("156 / 156", "formulas where the engine matches Excel exactly",
        ...
        ("182 / 182", "planted defects found by the right rule",
         "Eight kinds of defect injected into twelve clean workbooks, every mutant recalculated "
         "by Excel, then scored on whether the right rule named the right cell."),
        ("0", "findings on 12 clean workbooks, 1,059 formulas",
         "Blank spacer rows, subtotals rolled into a grand total, cross-sheet assumptions, "
         "VLOOKUP tables, hidden helper columns. A checker that cries wolf gets uninstalled."),
    ]
    ...
    card(s, 0.85, 4.9, 5.7, 1.75, fill=DANGER_WASH, line=DANGER_WASH)
    text(s, 1.15, 5.1, 5.1, 1.5, [
        ("The benchmark earned its keep.", 14, True, DANGER),
        ("It caught a real bug in Gridlint's own arithmetic: ROUND used a relative epsilon to "
         "correct binary representation error, which turned 3,806,241.4967 into 3,806,242. It "
         "now rounds in decimal.", 12, False, INK2),
    ])
    card(s, 6.8, 4.9, 5.7, 1.75, fill=BRAND_WASH, line=BRAND_WASH)
    text(s, 7.1, 5.1, 5.1, 1.5, [
        ("And a fourth number: 7 seconds.", 14, True, BRAND),
        ("A 23,000-formula model, audited end to end. Profiling it found two quadratic passes "
         "that had been costing 37 seconds. The evaluator runs at 88,000 formulas a second.",
         12, False, INK2),
    ])
    footer(s, 8)
...
def s10_roadmap(prs):
    ...
    built = [
        "Formula engine, 90 functions, 156/156 against Excel",
        "Dependency graph, recalculation, cycle detection",
        "15 rules, impact measured by recalculation",
        "Web app: workspaces, history, shared reports, corrected download",
        "Dependency trace: where a number actually comes from",
        "CLI, CI workflow, Docker, 151 tests",
    ]
    nxt = [
        "Google Sheets: read a file directly instead of exporting",
        "More of Excel: array formulas, dynamic arrays, LET and LAMBDA",
