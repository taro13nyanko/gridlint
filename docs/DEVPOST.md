...
3. Runs 15 deterministic rules to find defects.
...
| Decide whether something is a defect | Code — 15 rules |
...
**Real files are not tidy.** Newer functions are stored with an `_xlfn.` prefix. Dates are
serial numbers carrying a deliberate 1900 leap-year bug. `SUMPRODUCT((A="x")*B)` needs
element-wise arithmetic. `Table1[Amount]` has to be resolved against the table definition.
`[1]Budget!B4` cannot be computed at all, and the honest answer there is to say so rather
than produce a number. Every one of those broke the engine the first time it met one, and
each now has a test.
...
- **156 / 156** — the engine reproduces every value in a conformance workbook generated by
  Excel itself, including the cases re-implementations usually get wrong.
- **182 / 182** — every planted defect found by the right rule in the mutation benchmark,
  across eight defect kinds and twelve base workbooks, each mutant recalculated by Excel.
- **0** — findings across twelve clean workbooks totalling 1,059 formulas.
- **23,000 formulas audited in 7 seconds**, after profiling turned up two quadratic passes
  that had been costing 37.
- **151 tests**, green on Linux and Windows, Python 3.10 and 3.12, with no Excel installed.
